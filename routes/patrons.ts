...
export const getAllPatrons = async (req, res) => {
    ...
    db.collection('patreonTiers').find({ }).toArray((err, tiers) => {
        if (err) {
            log.WARN(err);
            res.status(err.code).send(err);
        }
        else {
            db.collection('patrons').find({ }).toArray((err, patrons) => {
                if (err) {
                    log.WARN(err);
                    res.status(err.code).send(err);
                }
                else {
                    tiers.map(tier => tier.list = patrons
                        .filter(patron => patron.tier === tier.tier))
                        .map(patron => patron = { steamid: patron.steamid, name: patron.name, avatar: patron.avatar });
...
export const getPatronsByTier = async (req, res) => {
    ...
    db.collection('patrons').find({ tier: req.params.tier }).toArray((err, tier) => {
        if (err) {
            log.WARN(err);
            res.status(err.code).send(err);
        }
...
export const getPatron = async (req, res) => {
    ...
    db.collection('patrons').findOne({ steamid: req.params.steamid }, (err, tier) => {
        if (err) {
            log.WARN(err);
            res.status(err.code).send(err);
        }
...
export const addPatron = async (req, res) => {
    ...
    db.collection('patrons').insertOne(patron, (err, data) => {
        if (err) {
            ...
            log.INFO(`Patron ${userSummary.data.response.players[0].name|| req.params.vanityid} added.`);
