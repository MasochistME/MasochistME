        ...
        axios.get(customUrl)
            .then(response => {
                ...
                if (completionRate == 100 
                    && cache.update.lastUpdate - (lastUnlockedAchievement*1000) <= 604800000
                    && cache.log.find(log => Number(log.player) === Number(cache.members[memberIndex].id) && Number(log.game) === Number(cache.members[memberIndex].games[gameIndex].appid) === undefined)
