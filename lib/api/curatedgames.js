        ...
        axios.get(url+id)
            .then(response => {
                if (response.status !== 200 || !response)
                    return reject(':c')
                games[index].title = response.data && response.data[id] && response.data[id].data
                    ? response.data[id].data.name
                    : "Title unknown";
                games[index].achievements = {
                    total: response.data && response.data[id] && response.data[id].data && response.data[id].data.achievements
                        ? response.data[id].data.achievements.total 
                        : 0,
                    list: [ ]
                };
                games[index].img = response.data && response.data[id] && response.data[id].data
                    ? response.data[id].data.header_image
                    : "";
