...
class Update {
     ...
     */
    call(req, res) {
        const updateCooldownMs = 43200000;
        if (server.updating) 
            return res.status(403).send({ content: "Already updating" });
        if (Date.now() - server.lastUpdate < updateCooldownMs) {
            const minutes = Math.round((updateCooldownMs - (Date.now() - server.lastUpdate)) % 60000);
            const hours = Math.round((updateCooldownMs - (Date.now() - server.lastUpdate))/60000);
            return res.status(403).send({ content: `Wait ${hours}:${minutes} hours before updating again!` })
        }
