...
export default function SectionTrivia() {
  const members = useSelector((state: any) => state.members.filter((member: any) => member.member));
    ...
    if (rating)
      return rating.map((tier: any, index: number) => 
        <li style={{ marginLeft: '30px' }} key={ `${tier.score}-${index}` }>
          ...
          <span className="bold">{ ` : ${ games.all.filter((game: any) => game.rating === tier.id).length }` }</span>
        </li>
      )
  }
  ...
  return(
    <Section>
      ...
      <p>Curated games:</p>
      <ul>
        <li style={{ marginLeft: '30px' }} >total: <span className="bold">{ games.total }</span></li>
      <ul>{ mapCurated() }</ul>
      </ul>
    </Section>
  )
