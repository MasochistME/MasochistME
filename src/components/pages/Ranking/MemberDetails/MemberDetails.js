...
export default class MemberDetails extends React.Component {
    ...
    render() {
        ...
        const classDisplay = show ? "member-details flex-column member-details-visible" : "member-details flex-column member-details-hidden"
        
        const composeGameList = () => {
            member.games = member.games.map(game => {
                game.completionRate = isNaN(game.completionRate) ? 0 : game.completionRate;
                return game;
            })
