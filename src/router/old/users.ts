...
const getUserGames = (userID: number, curatedGames: any, userToUpdate: any) =>
  new Promise(async (resolve, reject) => {
    ...
    games = games
      .filter(
        game => !!curatedGames.find(cachedgame => cachedgame.id == game.appid),
      )
      .map(game => {
        return {
          appid: game.appid,
    ...
    userGames
      ...
      .map(filteredgame => {
        const game = curatedGames.find(
          cachedgame => cachedgame.id == filteredgame.appid,
        );
...
export const updateUser = async (req, res) => {
  ...
  if (
    ...
    !userData.data.response.players
  ) {
  ...
  const gamesAsync = await getUserGames(
    req.params.steamid,
    curatedGames,
    userToUpdate,
  );
  ...
  db.collection('users').updateOne(
    ...
    (err, data) => {
      if (err) {
        ...
        const index = cache.updating.findIndex(
          user => user.user === req.params.steamid,
        );
